...
type Granularity = 'symbol' | 'character' | 'word' | 'sentence' | 'line' | 'paragraph' | 'lineboundary' | 'sentenceboundary' | 'paragraphboundary' | 'documentboundary';

export const isTextNode = (node: Node | null): node is Text => node ? node.nodeType === Node.TEXT_NODE : false;

const isText = (text?: string) => text && /[\w']/i.test(text);
const isSpace = (text?: string) => text && /[\s\t]/i.test(text);

const destructSelection = (selection: Selection) => {
...
const trimSelectionLeft = (selection: Selection) => {
...
const trimSelectionRight = (selection: Selection) => {
  const resultRange = selection.getRangeAt(0);
...
const trimSelection = (selection: Selection) => {
 ...
 */
const findBoundarySelection = (selection: Selection, boundary: string) => {
  const {
    range: originalRange,
    startOffset,
    startContainer,
...
const closestBoundarySelection = (selection: Selection, boundary: string) => {
  const {
    range: originalRange,
    startOffset,
    startContainer,
...
const boundarySelection = (selection: Selection, boundary: Granularity) => {
  const wordBoundary = boundary !== 'symbol' && boundary !== 'character';
  const {
    startOffset,
    startContainer,
    endOffset,
...
type CaptureCallback = (response: {selectionText: string, range: Range}) => void
type CaptureOptions = {
...
export const captureSelection = (callback: CaptureCallback, options: CaptureOptions = {}) => {
  const {
    granularity = 'symbol',
    win = window,
    beforeCleanup,
 ...
 */
const applyTextGranularity = (selection: Selection, granularity?: Granularity) => {
...
type TextNodeLookupDirection = 'forward' | 'backward' | 'forward-next' | 'backward-next';
const textNodeLookup = (
  commonContainer: Element,
  node: Element,
  offset: number,
  direction: TextNodeLookupDirection = 'forward',
) => {
...
const fixRange = (range: Range) => {
  ...
  let startContainer = range.startContainer as Text | null;
  let endContainer = range.endContainer as Text | null;
  ...
  const isBasicallyEmpty = (textNode: Text) => /^\s*$/.test(textNode.wholeText);
...
type ParametersExteptLast<T> = T extends (...args: infer U) => any
  ? U extends [...infer V, any]
    ? V
    : never
  : never;
 ...
 */
export const highlightRange = (range: Range, { label, classNames }: {
  label: string,
  classNames: string[],
}) => {
   ...
   */
  const applyStyledHighlight = (...args: ParametersExteptLast<typeof highlightRangePart>) => highlightRangePart(...args, classNames);
...
export const highlightRangePart = (
  ...
  if (startOffset === 0 && container.length === endOffset && parent.classList.contains(classNames[0])) {
    ...
    const parentNode = parent.parentNode as Element;
    ...
    const noseNode = container.cloneNode() as Text;
    const tailNode = container.cloneNode() as Text;
 ...
 * @param {string} [label]
 */
export const wrapWithSpan = (node: Text | Element, classNames: string[], label?: string) => {
 ...
 * @param {Node} root
 */
export const findNodesBetween = (startNode: Node, endNode: Node, root: Node) => {
  ...
  const walker = (root as Element).ownerDocument.createTreeWalker(root, NodeFilter.SHOW_ALL);
 ...
 */
export const removeRange = (spans: HTMLElement[]) => {
  if (!spans) return;
  spans.forEach(hl => {
    const fragment = hl.ownerDocument.createDocumentFragment();
    const parent = hl.parentNode as Element;
 ...
 * @param {Node} root
 */
export const findRange = (start: number, end: number, root: Node) => {
  return {
...
export const findRangeNative = (start: number, end: number, root: Node) => {
 ...
 */
export const codePointsToChars = (options?: {
  node: Node,
  position: number,
}) => {
  if (!options) return;
  ...
  const codePoints = [...text].slice(0, position);
 ...
 */
export const charsToCodePoints = (options: {
  node: Node,
  position: number,
}) => {
  const { node, position } = options;
 ...
 */
export const fixCodePointsInRange = (range: Range) => {
 ...
 */
export const findOnPosition = (root: Node, position: number, borderSide: 'left' | 'right' = 'left') => {
  const walker = getOwnerDocument(root).createTreeWalker(root, NodeFilter.SHOW_ALL);
 ...
 * @param {Node} root
 */
export const rangeToGlobalOffset = (range: Range, root: Node) => {
  const globalOffsets = [
 ...
 * @param {Node} root
 */
const findGlobalOffset = (node: Node, position: number, root: Node) => {
  const walker = getOwnerDocument(root).createTreeWalker(root, NodeFilter.SHOW_ALL);
  ...
  while (currentNode) {
    ...
    if (isTextNode(currentNode) || isBR) {
      let length = isDefined((currentNode as Text).length)
...
export const isSelectionContainsSpan = (spanNode: Element) => {
  const selection = window.getSelection();
  const spanRange = document.createRange();
  const textNode = spanNode.childNodes[0] as Text | undefined;
...
const getOwnerDocument = (node: Node) => {
  return (node as HTMLIFrameElement).contentDocument ?? node.ownerDocument ?? document;
};
