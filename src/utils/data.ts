 ...
 */
export const parseCSV = (text: string, separator = 'auto') => {
  ...
  if (separator === 'auto' && lines.length > 1) {
    ...
    if (!candidates || !candidates.length) throw new Error('No separators found');
  ...
  const split = (text: string) => text.trim().match(re) as string[] | null;

  // detect header; if it is omitted, use indices as a header names
  let names = split(lines[0])!;
 ...
 */
export const tryToParseJSON = (value: string | Record<string, any>) => {
 ...
 */
export const parseTypeAndOption = (valueType: string) => {
