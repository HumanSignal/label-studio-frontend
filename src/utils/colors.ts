 ...
 * @param {*} value
 */
export function colorToRGBA(value: string | undefined, alpha: number) {
  if (typeof value === 'string') {
    const hexColor = colorNames[value.toLowerCase() as keyof typeof colorNames];
 ...
 * @param {number} alpha
 */
export function convertToRGBA(value: string, alpha: number) {
 ...
 */
export function rgbaChangeAlpha(rgba: string, alpha: number) {
...
export function getScaleGradient(number: number) {
...
export const contrastColor = (color: string) => {
  ...
  const [r, g, b] = removeAlpha(...colors);
 ...
 * @param {string} color
 */
export function colorToRGBAArray(value: string) {
  ...
  if ((matches = RGBARegEx.exec(value))) {
    return matches.slice(1, 5).map(x => Number(x));
  }

  if ((matches = RGBRegEx.exec(value))) {
    const colorRGBArray = matches
      .slice(1, 4)
      .map(x => Number(x));
 ...
 */
export function rgbArrayToHex(value: number[]) {
  const color = value
    .slice(0, 3)
    .map(x => (x | (1 << 8))
    .toString(16)
...
export function rgbaArrayToRGBA(rgba: number[]) {
  return `rgba(${rgba[0]}, ${rgba[1]}, ${rgba[2]}, ${rgba[3]})`;
}

export function over(inputColor: string, inputBackgroundColor = 'white') {
  const color = chroma(inputColor);
  const bgColor = chroma(inputBackgroundColor);
