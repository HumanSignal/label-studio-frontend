    ...
    <Block name='annotations-carousel' style={{ '--carousel-left': `${currentPosition}px` }}>
      ...
      {(!isLeftDisabled || !isRightDisabled) && (
        <Elem name='carousel-controls'>
          <Elem tag={Button} name='nav' disabled={isLeftDisabled} mod={{ left: true }} aria-label="Carousel left" onClick={(e: MouseEvent) => !isLeftDisabled && updatePosition(e, true)}>
            <Elem name='arrow' mod={{ left: true }} tag={LsChevron} />
          </Elem>
          <Elem tag={Button} name='nav' disabled={isRightDisabled} mod={{ right: true }} aria-label="Carousel right" onClick={(e: MouseEvent) => !isRightDisabled && updatePosition(e, false)}>
