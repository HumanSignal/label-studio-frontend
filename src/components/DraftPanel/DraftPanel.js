...
import { LsAlert } from '../../assets/icons';
...
export const DraftPanel = observer(({ item }) => {
  if (!item.draftSaved && !item.versions.draft) {
    return null;
  }
  ...
  return (
    <div className={panel}>
      <LsAlert/>
      <Tooltip placement="topLeft" title={item.draft ? "switch to original result" : "switch to current draft"}>
        <button onClick={item.toggleDraft} className={panel.elem("toggle")}>
          Show {item.draft ? "draft" : "original"}
        </button>
      </Tooltip>
      {saved}
    </div>
