...
const TagAttrs = types.model({
  ...
  v1: types.maybeNull(types.string),
  v2: types.maybeNull(types.string),
...
const Model = types
  ...
  .views(self => ({
    ...
    get parent() {
      return Types.getParentOfTypeString(self, ["GraphLabelModel"]);
    },
  }))
  .actions(self => ({
    /**
...
const EdgeModel = types.compose("EdgeModel", TagAttrs, Model, ProcessAttrsMixin);

const HtxEdgeView = inject("store")(
  observer(({ item, store }) => {
...
Registry.addTag("edge", EdgeModel, HtxEdgeView);

export { HtxEdgeView, EdgeModel };
